...
const initSupabase = (cb)=>{
    ...
    if (localSessionInfo) fetch(`${(0, _config.BACKEND_BASE_URL)}/auth/getSession`, {
        method: "GET",
        headers: (0, _config.createReqHeaders)(localSessionInfo.token)
    }).then(async (res)=>{
        if (!res.ok || res.status === 401) throw new Error("Invalid session. Token expired.");
        if (res.ok) {
    ...
    else window.location.pathname, (0, _config.USER_PATHS).signIn;
};
...
parcelHelpers.export(exports, "forceLogout", ()=>forceLogout);
...
const forceLogout = ()=>{
    console.log((0, _config.USER_PATHS).signIn);
    localStorage.removeItem("@bw-user-auth");
    userAuth.logout();
    (0, _core.navigate)((0, _config.USER_PATHS).signIn);
};
...
parcelHelpers.export(exports, "createReqHeaders", ()=>createReqHeaders);
parcelHelpers.export(exports, "setRefreshToken", ()=>setRefreshToken);
parcelHelpers.export(exports, "getRefreshToken", ()=>getRefreshToken);
const USER_PATHS = {
    landing: "",
    authRoute: "/auth/(.*)",
    signIn: "/auth/sign-in",
...
const BACKEND_BASE_URL = window.location.hostname.includes("webflow.io") ? 'https://travelshore-backend-proxy.vercel.app/api' : 'http://localhost:3000/api';
const BACKEND_PROTECTED_BASE_URL = window.location.hostname.includes("webflow.io") ? 'https://travelshore-backend-proxy.vercel.app/api/protected' : 'http://localhost:3000/api/protected';
const REQ_HEADERS = {
    'Content-Type': 'application/json'
};
const createReqHeaders = (token = null)=>{
    if (token) return {
        ...REQ_HEADERS,
